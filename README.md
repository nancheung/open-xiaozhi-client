...
| 参数 | 说明 | 示例 |
| --- | --- | --- |
| `--ota=<url>` | 指定默认服务地址（OTA）。启动时注入前端，作为顶部「服务地址」输入框的默认值 | `--ota=https://your-server/xiaozhi/ota/` |
| `--port=<n>` | 指定监听端口。不指定时默认 `14100`，被占用会自动顺延；指定后若端口被占用则报错退出 | `--port=14200` |

```bash
open-xiaozhi-client-webui --ota=https://your-server/xiaozhi/ota/ --port=14200
```

> 说明：
> - 两种写法均可：`--ota <url>` 或 `--ota=<url>`（`--port` 同理）。
> - 服务默认监听 `0.0.0.0`，可通过局域网 IP 访问。
> - `--ota` 仅作为「服务地址」输入框的**默认值**；若浏览器本地已保存过手动填写的地址（localStorage），则以本地保存的为准。
